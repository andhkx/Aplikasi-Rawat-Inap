...
def cetak_transaksi():
    ...
    pdf.set_font('DejaVu', '', 16)
    pdf.set_text_color(40, 40, 40)
    pdf.cell(0, 12, 'Data Transaksi Rawat Inap', align='C', ln=1)
    pdf.ln(4)
    pdf.set_font('DejaVu', '', 12)
    pdf.set_fill_color(220, 220, 220)
    pdf.set_text_color(0, 0, 0)

    col_widths = [25, 25, 25, 30, 30, 30, 30]
    left_margin = (210 - sum(col_widths)) // 2
    pdf.set_x(left_margin)
    headers = ['ID Transaksi','ID Pasien','Total Biaya','Tgl Masuk','Tgl Keluar','Status','Tgl Struk']
    ...
    for d in data:
        ...
        row_vals = [
            str(d.get('id_transaksi') or ''),
            str(d.get('id_pasien') or ''),
            str(d.get('total_biaya') or ''),
            str(d.get('tgl_masuk') or ''),
            str(d.get('tgl_keluar') or ''),
            str(d.get('status_pembayaran') or ''),
            str(d.get('tgl') or '')
        ]

        for i, (w, txt) in enumerate(zip(col_widths, row_vals)):
            ln = 1 if i == (len(col_widths) - 1) else 0
            pdf.cell(w, 10, txt, 1, ln, 'C', fill)
        fill = not fill
...
@app.route('/cetak/transaksi/<id_transaksi>')
def cetak_satu_transaksi(id_transaksi):
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    query = """
    SELECT t.id_transaksi, t.id_pasien, t.total_biaya, r.tgl_masuk, r.tgl_keluar,
           t.status_pembayaran, t.tgl, p.nama
    FROM transaksi_dhika t
    JOIN rawatinap_dhika r ON t.id_pasien = r.id_pasien
    LEFT JOIN pasien_dhika p ON t.id_pasien = p.id_pasien
    ...
    pdf.set_font('DejaVu', '', 16)
    pdf.set_text_color(40, 40, 40)
    pdf.cell(0, 12, 'Struk Transaksi Rawat Inap', align='C', ln=1)
    pdf.ln(6)
    pdf.set_font('DejaVu', '', 12)

    label_w, value_w = 60, 120
    left_margin = (210 - (label_w + value_w)) // 2
    pdf.set_x(left_margin)

    pdf.set_fill_color(220, 220, 220)
    fill = True
    rows = [
        ('ID Transaksi', d.get('id_transaksi') or ''),
        ('ID Pasien', d.get('id_pasien') or ''),
        ('Nama Pasien', d.get('nama') or ''),
        ('Total Biaya', str(d.get('total_biaya') or '')),
        ('Tgl Masuk', str(d.get('tgl_masuk') or '')),
        ('Tgl Keluar', str(d.get('tgl_keluar') or '')),
        ('Status Pembayaran', d.get('status_pembayaran') or ''),
        ('Tgl Struk', str(d.get('tgl') or ''))
    ]

    for label, val in rows:
        pdf.set_x(left_margin)
        pdf.set_fill_color(220,220,220) if fill else pdf.set_fill_color(255,255,255)
        pdf.cell(label_w, 10, label, 1, 0, 'L', True)
        pdf.cell(value_w, 10, val, 1, 1, 'L', fill)
        fill = not fill
    ...
    response = make_response(pdf_bytes)
    response.headers['Content-Type'] = 'application/pdf'
    response.headers['Content-Disposition'] = f'attachment; filename=struk_transaksi_{id_transaksi}.pdf'
    return response
